...
CREATE TABLE lesson_t (
    ID UUID PRIMARY KEY ,
    course_ID UUID NOT NULL,
    date DATE NOT NULL,
    time_interval VARCHAR(11),
    digit_code_id UUID,
    FOREIGN KEY (course_ID) REFERENCES COURSE_T(ID) ON DELETE CASCADE,
    FOREIGN KEY (digit_code_id) REFERENCES DIGIT_CODE_T(digit_code_id) ON DELETE CASCADE
);
...
FOREIGN KEY (lesson_id) REFERENCES lesson_t(id);
